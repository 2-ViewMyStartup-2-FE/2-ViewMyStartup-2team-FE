    ...
    <div className={style.table}>
      {data.map((item, index) => (
        <div
          ...
          <div
            className={`${style.investment} ${isStatusPage ? style.status : ""}`} >
            {!isStatusPage
              ? ConvertBillion(parseInt(item.totalInvestment))
              : !isCompareStatus
              ? ConvertBillion(parseInt(item.virtualInvestment))
              : item.myChosenCount}
          </div>
          <div
            className={`${style.revenue} ${isStatusPage ? style.status : ""}`} >
            {!isStatusPage
              ? ConvertBillion(parseInt(item.revenue))
              : !isCompareStatus
              ? ConvertBillion(parseInt(item.actualInvestment))
              : item.comparedChosenCount}
          </div>
          {!isStatusPage && (
